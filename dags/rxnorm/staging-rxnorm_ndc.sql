...
FROM datasource.rxnsat rxnsat
	INNER JOIN datasource.rxnconso rxnconso ON rxnsat.rxaui = rxnconso.rxaui
	LEFT JOIN datasource.rxnrel sbdrel ON rxnsat.RXCUI = sbdrel.RXCUI2 AND RELA = 'tradename_of' and rxnconso.tty IN ('BPCK','SBD')
	LEFT JOIN datasource.rxnconso relc ON sbdrel.RXCUI1 = relc.RXCUI AND relc.tty IN ('SCD','GPCK')
