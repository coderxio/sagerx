...
-- NOTE: this requires both staging.fda_ndc and datasource.orange_book 
-- DISCLAIMER: because I can't figure out NDC-level mapping, I only include applications with a single OB TE code
CREATE OR REPLACE VIEW flatfile.ndc_to_orange_book_te_code
AS 
    WITH cte AS (
        SELECT
            fda.ndc11
        ...
        INNER JOIN staging.fda_ndc fda ON concat(CASE WHEN obp.appl_type = 'A' THEN 'ANDA' ELSE 'NDA' END, obp.appl_no) = fda.applicationnumber
