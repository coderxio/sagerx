...
def main_execution():
    ...
    """
    logging.info("Starting data retrieval for RxClass/EPC...")
    ...
    class_base_url = (
        ...
        "classTypes=EPC&relaSource=DAILYMED&rela=has_EPC&rxcui="
    )
    ...
    # 6. Filter rows with classType == 'EPC'
    full_df = full_df[full_df["classType"] == "EPC"].reset_index(drop=True)
    ...
    load_df_to_pg(full_df, "sagerx_lake", "rxnorm_epc", "replace")
...
def rxnorm_epc():
    """
    DAG that retrieves RxClass concepts for EPC, rate-limited at 20 calls/second.
    Structured similarly to the 'rxnorm_epc' DAG pattern.
    """
